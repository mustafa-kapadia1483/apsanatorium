<script>
	...
	const guestContactListData = data.guestContactListData;
</script>

<div class="container border px-0 border-gray-200 mb-4">
	<GradientHeader>Guest Contact List</GradientHeader>
	<form
		class="w-3/4 mx-auto py-4 grid place-items-center"
		action="/dashboard/reports/working-reports/guest-contact-list"
	...
	{#if guestContactListData}
		<ReportTable
			title={`Guest Contact List from ${strftime('%d-%b-%Y', new Date(data.params.startDate))} to ${strftime('%d-%b-%Y', new Date(data.params.endDate))}`}
			tableBody={guestContactListData}
			containerClass="mt-4"
		/>
	{/if}
</div>
