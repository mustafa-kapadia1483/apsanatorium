...
<div class="container border px-0 border-gray-200">
	<GradientHeader>Short Invoice Report</GradientHeader>
	<form
		class="w-3/4 mx-auto my-3"
		action="/dashboard/reports/accounting-reports/short-invoice-report"
	...
	{#if shortStayReportData}
		<ReportTable
			...
			fileName="short-invoice-report"
			title={`Short Invoice Report From Date: ${strftime('%d-%b-%Y', new Date(data.startDate))} To: ${strftime(
				'%d-%b-%Y',
